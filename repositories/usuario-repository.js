...
class usuarioRepository{
    constructor(){
        this._base = new base('Usuario'); //utilizando a const base
        this._projection = 'nome email _id';
    }

    async IsEmailExiste(Email){

        return await this._base._model.findOne({email: Email}, this._projection);// vai reronar nome email _id
    }

    async authenticate(Email,Senha){
        let _hashSenha = md5(Senha);
        return await this._base._model.findOne({email: Email, senha:_hashSenha}, this._projection);// vai reronar nome email _id
    }
    ...
    async update(id, data){
        let usuarioAtualizado = await this._base.update(id, { // this._base.update(id, data);
            nome: data.nome,
            email: data.email,
            foto: data.foto
        });
